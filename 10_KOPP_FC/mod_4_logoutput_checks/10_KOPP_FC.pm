...
#
# https://wiki.fhem.de/wiki/Kopp_Allgemein | https://forum.fhem.de/index.php/topic,47846.msg407600.html#msg407600
#
#
# Date        Who                     Comment
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# 2021-02-05  RaspII,HomeAutoUser     revised KOPP_FC_Define / KOPP_FC_Undef / KOPP_FC_Attr logoutputs & checks
# 2021-02-04  RaspII,HomeAutoUser     Creation of systematic order (comments, code overview)
...
sub KOPP_FC_Initialize($) {
  ...
  $hash->{Match}    = "^kr.*";          # Anpassung, ggf auf "^kr.................." damit nur genau 19 Zeichen akzeptiert werden (oder besseres RegEx)
  $hash->{SetFn}    = "KOPP_FC_Set";
  $hash->{DefFn}    = "KOPP_FC_Define";
  $hash->{UndefFn}  = "KOPP_FC_Undef";
  $hash->{ParseFn}  = "KOPP_FC_Parse";
  $hash->{AttrFn}   = "KOPP_FC_Attr";
  $hash->{AttrList} = "IODev ".
...
sub KOPP_FC_Define($$) {
  ...
  if ( scalar(@a) < 5 || scalar(@a) > 7) {
    return "wrong syntax: define <name> KOPP_FC <Keycode (Byte)> <Transmittercode1 (2Byte)> <Transmittercode2 (Byte)> <optional Keycode 2 (Byte)> <optional Keycode 3 (Byte)>";
  }
  ...
  if (defined $a[5]){
    if ( ( $a[5] !~ m/^[a-fA-F0-9]{2}$/i ) ) {     # Default: Keycode2 is empty
      return "Define $a[0]: wrong keycode2 format: specify a 2 digit hex value";
    }
    else
    {
      $keycode2 = $a[5];
  ...
  if (defined $a[6]){
    if ( ( $a[6] !~ m/^[a-fA-F0-9]{2}$/i ) ) {     # Default: Keycode3 is empty
      return "Define $a[0]: wrong keycode3 format: specify a 2 digit hex value";
    }
    else
    {
      $keycode3 = $a[6];
  ...
  my $logoutput = "";
  $logoutput.= " | Keycode2: " . $hash->{KEYCODE2} if(defined $a[5]);
  $logoutput.= " | Keycode3: " . $hash->{KEYCODE3} if(defined $a[6]);

  Log3 $name, 2, "$name: Define, with Keycode: $keycode | Transmittercode1: " . $hash->{TRANSMITTERCODE1} . " | Transmittercode2: " . $hash->{TRANSMITTERCODE2} . "$logoutput";
  ...
  foreach my $c (keys %{ $hash->{CODE} } ) {
    ...
    foreach my $dname (keys %{ $modules{KOPP_FC}{defptr}{$c} }) {
      if($modules{KOPP_FC}{defptr}{$c}{$dname} == $hash) {
        Log3 $name, 5, "$name: Undef, Code: $c, " . $modules{KOPP_FC}{defptr}{$c}{$dname} . " deleted"; # for DEBUG
        delete($modules{KOPP_FC}{defptr}{$c}{$dname});
      }
    }
  }
  return undef;
...
sub KOPP_FC_Attr(@) {
  ...
  # aName are Attribute name
  # aVal are Attribute value
...
sub KOPP_FC_SendCommand($@) {
  ...
  $message = "s" . $keycodehex . $hash->{TRANSMITTERCODE1} . $hash->{TRANSMITTERCODE2} . $hash->{TIMEOUT} . "N";    # N for do not print messages (FHEM will write error messages to log files if CCD/CUL sends status info
...
sub KOPP_FC_Set($@) {
  ...
  if(!defined($c)) {
    # if set command was not yet defined in %codes provide command list
  ...
  if(defined($attr{$name}) && defined($attr{$name}{"model"})) {
    # Falls Model spezifiziert ist, Model ermitteln -> $mt
    # ----------------------------------------------------
