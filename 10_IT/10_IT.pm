...
IT_Set($@)
{
  ...
  if ($hash->{READINGS}{protocol}{VAL} eq "V3") {
    ...
    if (length($onoffcode) == 4 && $hash->{READINGS}{protocol}{VAL} ne 'SBC_FreeTec' && AttrVal($name,'model','') ne "yet100") {   # EV1527, YET100-1
			Log 5, "ITSet: onoffcode before bintotristate $onoffcode";
      $onoffcode = $bintotristate{substr($onoffcode,0,2)} . $bintotristate{substr($onoffcode,2,2)};
			Log 5, "ITSet: onoffcode after bintotristate  $onoffcode";
    }
...
sub IT_Attr(@)
{
  ...
  if ($cmd eq 'set') {
    if( $aName eq 'model') {
      ...
      if ( ReadingsVal($name, 'protocol', '') ne 'V1' && ReadingsVal($name, 'protocol', '') ne 'EV1527' ) { # see sub IT_Set
        $ret = "ERROR: not supported on this protocol! (only protocol V1 and EV1527)\nYou can change this with attribute protocol or model.";
      }
    ...
    } elsif ( $aName eq 'protocol') {
      CommandSetReading(undef, "$name protocol $aVal"); # to set right Readings value protocol!
    }
  
  } elsif ($cmd eq 'del') {
    if ($aName eq 'userV1setCodes') { # to delete userV1setCodes from setlist 
      delete $hash->{userV1setCodes};
    }
  }
